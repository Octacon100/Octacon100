...
#Loop through the seat ids after sorting them, checking for a number that is skipped:
seatIdList.sort()
for i in range(0, len(seatIdList) - 1):
    if seatIdList[i + 1] != (int(seatIdList[i]) + 1):
        possibleSeat = int(seatIdList[i]) + 1
        print (str(possibleSeat) + " is hopefully your seat.")
